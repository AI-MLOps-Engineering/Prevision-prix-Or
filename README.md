...
| CI/CD | GitHub Actions (tests, lint, build Docker, push GHCR, déploiement SSH optionnel) |
...
- Copier **`.env.example`** vers **`.env`** si le projet en fournit un, ou créer un **`.env`** local (ignoré par Git) pour les secrets applicatifs.
- Le fichier **`.env`** ne doit pas être versionné.

---

*Projet orienté comparaison de modèles et mise en production sur VPS Scaleway ; les prévisions TST non entraînées sur mesure restent indicatives.*
